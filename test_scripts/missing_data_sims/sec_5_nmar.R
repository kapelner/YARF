...
results_yarf_nmar = matrix(NA, nrow = length(approx_prop_missing), ncol = Nsim)
results_yarf_w_rfi_and_mf_nmar = matrix(NA, nrow = length(approx_prop_missing), ncol = Nsim)
results_rf_nmar = matrix(NA, nrow = length(approx_prop_missing), ncol = Nsim)
rownames(results_yarf_nmar) = approx_prop_missing
rownames(results_yarf_w_rfi_and_mf_nmar) = approx_prop_missing
...
for (nsim in 1 : Nsim){
	...
	for (g in 1 : length(betas)){
		...
		results_yarf_nmar[g, nsim] = calc_rmse(predict(yarf_mod, Xtest), ytest)
		results_yarf_w_rfi_and_mf_nmar[g, nsim] = calc_rmse(predict(yarf_mod_rf_imp, Xtest_miss_rf), ytest)
		results_rf_nmar[g, nsim] = calc_rmse(predict(rf_mod, Xtest_miss_rf), ytest)
		
		cat("yarf oosrmse:", results_yarf_nmar[g, nsim], "rf oosrmse:", results_rf_nmar[g, nsim], "yarf_with_rf_imp oosrmse:", results_yarf_w_rfi_and_mf_nmar[g, nsim], "\n")
		
		avgs_nmar_yarf = apply(results_yarf_nmar, 1, mean, na.rm = TRUE)		
		rel_nmar_avgs_yarf = avgs_nmar_yarf / avgs_nmar_yarf[1]
		sd_nmar_yarf = apply(results_yarf_nmar / avgs_nmar_yarf[1], 1, sd, na.rm = TRUE)
		
		avgs_nmar_yarf_w_rfi_and_mf = apply(results_yarf_w_rfi_and_mf_nmar, 1, mean, na.rm = TRUE)		
		rel_nmar_avgs_yarf_w_rfi_and_mf = avgs_nmar_yarf_w_rfi_and_mf / avgs_nmar_yarf[1]
		sd_nmar_yarf_w_rfi_and_mf = apply(results_yarf_w_rfi_and_mf_nmar / avgs_nmar_yarf[1], 1, sd, na.rm = TRUE)
		
		avgs_nmar_rf = apply(results_rf_nmar, 1, mean, na.rm = TRUE)		
		rel_nmar_avgs_rf = avgs_nmar_rf / avgs_nmar_yarf[1]
		sd_nmar_rf = apply(results_rf_nmar / avgs_nmar_yarf[1], 1, sd, na.rm = TRUE)
		...
		plot(approx_prop_missing, 
				...
				ylim = c(min(rel_nmar_avgs_yarf, rel_nmar_avgs_yarf_w_rfi_and_mf, rel_nmar_avgs_rf, na.rm = TRUE), max(rel_nmar_avgs_yarf, rel_nmar_avgs_yarf_w_rfi_and_mf, rel_nmar_avgs_rf, na.rm = TRUE)))
		for (i in 1 : length(approx_prop_missing)){
			x = approx_prop_missing[i]
			y_plot = rel_nmar_avgs_yarf[i]
			moe = 1.96 * sd_nmar_yarf[i] / sqrt(nsim)
			segments(x, y_plot - moe, x, y_plot + moe, col = "green")
		}
		points(approx_prop_missing, rel_nmar_avgs_yarf_w_rfi_and_mf, col = "blue", type = "o")
		for (i in 1 : length(approx_prop_missing)){
			x = approx_prop_missing[i]
			y_plot = rel_nmar_avgs_yarf_w_rfi_and_mf[i]
			moe = 1.96 * sd_nmar_yarf_w_rfi_and_mf[i] / sqrt(nsim)
