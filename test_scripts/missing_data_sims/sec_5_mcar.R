...
results_yarf_mcar = matrix(NA, nrow = length(approx_prop_missing), ncol = Nsim)
results_yarf_w_rfi_and_mf_mcar = matrix(NA, nrow = length(approx_prop_missing), ncol = Nsim)
results_rf_mcar = matrix(NA, nrow = length(approx_prop_missing), ncol = Nsim)
rownames(results_yarf_mcar) = approx_prop_missing
rownames(results_yarf_w_rfi_and_mf_mcar) = approx_prop_missing
...
for (nsim in 1 : Nsim){
	...
	for (g in 1 : length(gammas)){
		...
		results_yarf_mcar[g, nsim] = calc_rmse(predict(yarf_mod, Xtest), ytest)
		results_yarf_w_rfi_and_mf_mcar[g, nsim] = calc_rmse(predict(yarf_mod_rf_imp, Xtest_miss_rf), ytest)
		results_rf_mcar[g, nsim] = calc_rmse(predict(rf_mod, Xtest_miss_rf), ytest)
		
		cat("yarf oosrmse:", results_yarf_mcar[g, nsim], "rf oosrmse:", results_rf_mcar[g, nsim], "yarf_with_rf_imp oosrmse:", results_yarf_w_rfi_and_mf_mcar[g, nsim], "\n")
		
		#rolling updates!!
		
		avgs_mcar_yarf = apply(results_yarf_mcar, 1, mean, na.rm = TRUE)		
		rel_mcar_avgs_yarf = avgs_mcar_yarf / avgs_mcar_yarf[1]
		sd_mcar_yarf = apply(results_yarf_mcar / avgs_mcar_yarf[1], 1, sd, na.rm = TRUE)
		
		avgs_mcar_yarf_w_rfi_and_mf = apply(results_yarf_w_rfi_and_mf_mcar, 1, mean, na.rm = TRUE)		
		rel_mcar_avgs_yarf_w_rfi_and_mf = avgs_mcar_yarf_w_rfi_and_mf / avgs_mcar_yarf[1]
		sd_mcar_yarf_w_rfi_and_mf = apply(results_yarf_w_rfi_and_mf_mcar / avgs_mcar_yarf[1], 1, sd, na.rm = TRUE)
		
		avgs_mcar_rf = apply(results_rf_mcar, 1, mean, na.rm = TRUE)		
		rel_mcar_avgs_rf = avgs_mcar_rf / avgs_mcar_yarf[1]
		sd_mcar_rf = apply(results_rf_mcar / avgs_mcar_yarf[1], 1, sd, na.rm = TRUE)
		
		par(mar = c(4.2,4,0.2,0.2))
		plot(approx_prop_missing, 
				rel_mcar_avgs_yarf[1 : length(approx_prop_missing)],
				type = "o", 
				xlab = "Proportion Missing",
				ylab = "Multiple of Baseline Error",
				ylim = c(min(rel_mcar_avgs_yarf, rel_mcar_avgs_yarf_w_rfi_and_mf, rel_mcar_avgs_rf, na.rm = TRUE), max(rel_mcar_avgs_yarf, rel_mcar_avgs_yarf_w_rfi_and_mf, rel_mcar_avgs_rf, na.rm = TRUE)))
		for (i in 1 : length(approx_prop_missing)){
			x = approx_prop_missing[i]
			y_plot = rel_mcar_avgs_yarf[i]
			moe = 1.96 * sd_mcar_yarf[i] / sqrt(nsim)
			segments(x, y_plot - moe, x, y_plot + moe)
		}
		points(approx_prop_missing, 
				rel_mcar_avgs_yarf_w_rfi_and_mf[1 : length(approx_prop_missing)], 
				lty = "dotted", 
				type = "o")
		for (i in 1 : length(approx_prop_missing)){
			x = approx_prop_missing[i]
			y_plot = rel_mcar_avgs_yarf_w_rfi_and_mf[i]
			moe = 1.96 * sd_mcar_yarf_w_rfi_and_mf[i] / sqrt(nsim)
			segments(x, y_plot - moe, x, y_plot + moe)
		}
		points(approx_prop_missing, 
